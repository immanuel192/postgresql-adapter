...
const Adapter = {
   ...
   * @param cb
   */
  define (connectionName, _tableName, definition, cb, collection) {
    let cxn = Adapter.connections.get(connectionName)
    let tableName = _tableName.substring(0, 63)
    let schemaName = ''
    
    if (collection && collection.query && collection.query.meta && collection.query.meta.schemaName){
      schemaName = collection.query.meta.schemaName;
    }
    ...
    return cxn.knex.schema
      ...
      .then(exists => {
        ...
        let createTableFunction = function (){
        ...
        // check schemaName
        if (schemaName != ''){
            return cxn.knex.schema.createSchemaIfNotExists(schemaName).then(createTableFunction)
        }
